...
mod tests {
    ...
    async fn builtin() {
        ...
        // When
        let result = publisher.try_publish(&payload).await;
        let received = subscriber.receive_many().await;

        // Then
        assert!(result.is_ok());
        assert_eq!(received, multiset(&[payload]));
    ...
    fn make_publisher() -> TestPublisher {
        prepare_publisher(|egress| {
            egress
                .with_exchange(Exchange::AmqFanout.name())
    ...
    async fn make_subscriber(names: &ExQu) -> PayloadSubscriber {
        prepare_subscriber(|ingress| {
            ingress
                .with_exchange(Exchange::AmqFanout)
